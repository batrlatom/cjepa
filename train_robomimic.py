    ...
    try:
        for epoch in range(args.epochs):
            ...
            for imgs, prop_t, act_t in tqdm(train_dl, desc=f"Epoch {epoch+1}"):
                ...
                # 3. Predict Actions
                pred_actions = policy(ctx_latents)  # (B, pred_horizon, 7)
            ...
            with torch.no_grad():
                for imgs, prop_t, act_t in val_dl:
                    ...
                    pred_actions = policy(ctx_latents)
                    loss = criterion(pred_actions, target_actions)
                    val_loss += loss.item()

            val_loss /= len(val_dl)

            print(f"Epoch [{epoch+1}/{args.epochs}] Time: {time.time()-start_t:.1f}s | Train MSE: {train_loss:.4f} | Val MSE: {val_loss:.4f}")
